...
open class BaseRepository {
    ...
    private suspend fun<T : Any> marvelApiOutput(call: suspend()-> Response<T>, error: String) : Output<T>{
        val response = call.invoke()
        return if (response.isSuccessful)
            Output.Success(response.body()!!)
        else
            Output.Error(IOException("Ops .. Something went wrong due to  ${response.code()} and error $error"))
    }
}